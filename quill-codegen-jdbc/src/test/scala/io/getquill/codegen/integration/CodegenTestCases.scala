...
sealed trait CodegenTestCases {
  ...
  protected def pathList(implicit dbPrefix: ConfigPrefix) = List("io", "getquill", "codegen", "generated", dbPrefix.packagePath)
...
object CodegenTestCases {
  ...
  case object `1-simple-snake` extends TestInfo(SnakeCase, simpleSnake) with `1-simple-snake` {
    override def generate(dbPrefix: ConfigPrefix, basePath: String) = {
      ...
      val snakeCaseGen = new SimpleJdbcCodegen(dbPrefix.loadConfig, `package`) with DatabaseBasedNumericTyping {
        ...
        override def filter(tc: RawSchema[JdbcTableMeta, JdbcColumnMeta]): Boolean = schemaFilter(databaseType, tc) && super.filter(tc)
  ...
  case object `1-comp-sanity` extends TestInfo(SnakeCase, simpleSnake) with `1-comp-sanity` {
    override def generate(dbPrefix: ConfigPrefix, basePath: String) = {
      implicit val prefix = dbPrefix
      implicit val base = basePath
      val gen = new ComposeableTraitsJdbcCodegen(dbPrefix.loadConfig, `package`, false) with DatabaseBasedNumericTyping {
        override def defaultNamespace: String = "public"
  ...
  case object `4-comp-stereo-twoschema` extends TestInfo(SnakeCase, twoSchema) with `4-comp-stereo-twoschema` {
    override def generate(dbPrefix: ConfigPrefix, basePath: String) = {
      implicit val prefix = dbPrefix
      implicit val base = basePath
      val gen = new ComposeableTraitsJdbcCodegen(databaseConnections, `package`, false) with DatabaseBasedNumericTyping {
        override def defaultNamespace: String = "public"
        override def nameParser: NameParser = CustomNames()
        override def filter(tc: RawSchema[JdbcTableMeta, JdbcColumnMeta]): Boolean = schemaFilter(databaseType, tc) && super.filter(tc)
        override def querySchemaNaming: JdbcQuerySchemaNaming = ts => {
        ...
        override def namespacer: Namespacer[JdbcTableMeta] = ts => {
          val schema = super.namespacer(ts)
          if (schema == "alpha" || schema == "bravo") "common" else {
            databaseType match {
  ...
  case object `5-comp-non-stereo-allschema` extends TestInfo(SnakeCase, twoSchema) with `5-comp-non-stereo-allschema` {
    override def generate(dbPrefix: ConfigPrefix, basePath: String) = {
      implicit val prefix = dbPrefix
      implicit val base = basePath
      val gen = new ComposeableTraitsJdbcCodegen(databaseConnections, `package`, nestedTrait = true) with DatabaseBasedNumericTyping {
        override def defaultNamespace: String = "public"
        override def filter(tc: RawSchema[JdbcTableMeta, JdbcColumnMeta]): Boolean = schemaFilter(databaseType, tc) && super.filter(tc)
        override def nameParser: NameParser = CustomNames()
        override def namespacer: Namespacer[JdbcTableMeta] = ts => {
  ...
  /**
   * Oracle does not really have any 32/64 bit Integer types so basically any Integer column that
   * you create will be a BigInteger. Specifically for Oracle, use a behavior that will attempt to use a
   * primitive datatype based on on the scale of the column.
   */
  ...
  private[integration] def databaseConnections(implicit prefix: ConfigPrefix) = prefix match {
    case TestOracleDB => Seq(
      JdbcContextConfig(prefix.loadConfig).dataSource,
      JdbcContextConfig(prefix.loadConfig.withValue("dataSource.user", ConfigValueFactory.fromAnyRef("alpha"))).dataSource,
      JdbcContextConfig(prefix.loadConfig.withValue("dataSource.user", ConfigValueFactory.fromAnyRef("bravo"))).dataSource
    ).map { ds => () => ds.getConnection }
    case TestSqlServerDB => Seq(
      JdbcContextConfig(prefix.loadConfig).dataSource,
      JdbcContextConfig(prefix.loadConfig.withValue("dataSource.databaseName", ConfigValueFactory.fromAnyRef("alpha"))).dataSource,
      JdbcContextConfig(prefix.loadConfig.withValue("dataSource.databaseName", ConfigValueFactory.fromAnyRef("bravo"))).dataSource
    ).map { ds => () => ds.getConnection }
    case _ => Seq(() =>
      JdbcContextConfig(prefix.loadConfig).dataSource.getConnection)
  }

  private[integration] def schemaFilter(databaseType: DatabaseType, tc: RawSchema[JdbcTableMeta, JdbcColumnMeta]) = databaseType match {
    case MySql =>
