...
class SqlQueryApply(traceConfig: TraceConfig) {
  ...
  private def flatten(sources: List[FromContext], finalFlatMapBody: Ast, alias: String, nestNextMap: Boolean): FlattenSqlQuery = {

    def select(alias: String, quat: Quat) = SelectValue(Ident(alias, quat), None) :: Nil

    def base(q: Ast, alias: String, nestNextMap: Boolean): FlattenSqlQuery = trace"Computing Base (nestingMaps=${nestNextMap}) for Query: $q" andReturn {
      ...
      q match {
        ...
        case q @ (_: Filter | _: Entity)   => trace"base| Flattening Filter/Entity $q" andReturn { flatten(sources, q, alias, nestNextMap) }
        ...
        case q if (sources == Nil)         => trace"base| Flattening Empty-Sources $q" andReturn { flatten(sources, q, alias, nestNextMap) }
    ...
    trace"Flattening (alias = $alias) sources $sources from $finalFlatMapBody" andReturn {
      finalFlatMapBody match {
        ...
        case GroupByMap(q, x @ Ident(alias, _), g, a, p) => trace"Flattening| GroupByMap" andReturn {
          val b = base(q, alias, true)
  ...
  private def orderByCriteria(ast: Ast, ordering: Ast, from: List[FromContext]): List[OrderByCriteria] =
    (ast, ordering) match {
      ...
      case (Tuple(properties), TupleOrdering(ord))    => properties.zip(ord).flatMap { case (a, o) => orderByCriteria(a, o, from) }
      ...
      case (id @ Ident(_, _: Quat.Product), ord)      => new ExpandSelection(from).ofSubselect(List(SelectValue(ast))).map(_.ast).flatMap(orderByCriteria(_, ord, from))
