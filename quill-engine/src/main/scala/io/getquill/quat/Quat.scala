...
sealed trait Quat {
  ...
  def lookup(path: String, failNonExist: Boolean): Quat = (this, path) match {
    ...
    case (Quat.Generic, fieldName) =>
      io.getquill.util.Messages.trace(s"The field '${fieldName}' was looked up from a Generic Quat. Assuming it will also be Quat.Generic", traceType = TraceType.Warning)
      Quat.Unknown
    case (other, fieldName) =>
      if (failNonExist)
