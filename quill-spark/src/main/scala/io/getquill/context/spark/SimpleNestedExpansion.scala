 ...
 *
 *  - unless the Ident has a Concrete Quat.Product with a single value,
 * but that has already been expanded into it's composite elements in TopLevelExpansion.apply and the Ident shuold no
