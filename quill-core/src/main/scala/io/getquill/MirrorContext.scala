 ...
 * needs to understand that fact e.g. `Decoder[Option[Int]](java.sql.ResultSet(foo:1, etc)).getInt(1)`* and wrap it into a Optional value
 * for the equivalent row implementation: `Decoder[Option[Int]](Row(foo:1, etc)).apply(1)`.
